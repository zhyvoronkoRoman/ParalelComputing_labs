...
import org.springframework.http.server.reactive.ServerHttpRequest; // ❗ Це важливо, має бути reactive
...
public class AuthenticationFilter extends AbstractGatewayFilterFactory<AuthenticationFilter.Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            // Безпечно отримуємо заголовок (Spring 4/WebFlux Style)
            ...
            if (authHeader.startsWith("Bearer ")) {
                ...
                // Витягуємо ID та конвертуємо в рядок
                String userId = String.valueOf(jwtUtils.getClaims(token).get("userId"));

                // Мутуємо запит, додаючи новий заголовок для мікросервісів
